...
public:
	...
	virtual void AddReferencedObjects(UObject* InThis, FReferenceCollector& Collector) override
	{
		...
		{
			UClass* Class = It.Key();
			TObjectPtr<UObject> SmartClass = Class;
			...
			{
				Collector.AddReferencedObject(SmartClass, InThis);
			}
		}
		...
		{
			TObjectPtr<UObject> SmartStruct = It.Key();
			Collector.AddReferencedObject(SmartStruct, InThis);
		}
	}
